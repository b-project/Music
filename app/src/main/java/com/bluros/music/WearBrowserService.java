...
import com.bluros.music.dataloaders.AlbumLoader;
import com.bluros.music.dataloaders.AlbumSongLoader;
import com.bluros.music.dataloaders.ArtistAlbumLoader;
import com.bluros.music.dataloaders.ArtistLoader;
import com.bluros.music.dataloaders.ArtistSongLoader;
import com.bluros.music.dataloaders.PlaylistLoader;
import com.bluros.music.dataloaders.PlaylistSongLoader;
import com.bluros.music.dataloaders.SongLoader;
import com.bluros.music.models.Album;
import com.bluros.music.models.Artist;
import com.bluros.music.models.Playlist;
...
public class WearBrowserService extends MediaBrowserService {
    ...
    private final class MediaSessionCallback extends MediaSession.Callback {
        ...
        public void onPlayFromMediaId(final String mediaId, Bundle extras) {
            ...
            setSessionActive();
            MusicPlayer.playAll(mContext, new long[]{songId}, 0, -1, MusicUtils.IdType.NA, false);
        }
        ...
        mMediaRoot.add(new MediaBrowser.MediaItem(
                new MediaDescription.Builder()
                        ...
                        .setIconUri(Uri.parse("android.resource://" +
                                "bluros.music/drawable/ic_empty_music2"))
                    ...
                    switch (Integer.parseInt(Character.toString(parentId.charAt(0)))) {
                        case TYPE_ARTIST:
                            ...
                            for (Artist artist : artistList) {
                                ...
                                fillMediaItems(mediaItems, Integer.toString(TYPE_ARTIST_SONG_ALBUMS) + Long.toString(artist.id), artist.name, Uri.parse("android.resource://" +
                                        "bluros.music/drawable/ic_empty_music2"), MusicUtils.makeCombinedString(mContext, albumNmber, songCount), MediaBrowser.MediaItem.FLAG_BROWSABLE);
                            }
                            break;
                        case TYPE_ALBUM:
                            List<Album> albumList = AlbumLoader.getAllAlbums(mContext);
                            for (Album album : albumList) {
                                fillMediaItems(mediaItems, Integer.toString(TYPE_ALBUM_SONGS) + Long.toString(album.id), album.title, MusicUtils.getAlbumArtUri(album.id), album.artistName, MediaBrowser.MediaItem.FLAG_BROWSABLE);
                            }
                            break;
                        case TYPE_SONG:
                            List<Song> songList = SongLoader.getAllSongs(mContext);
                            for (Song song : songList) {
                                fillMediaItems(mediaItems, String.valueOf(song.id), song.title, MusicUtils.getAlbumArtUri(song.albumId), song.artistName, MediaBrowser.MediaItem.FLAG_PLAYABLE);
                            }
                            break;
                        ...
                        case TYPE_ARTIST_SONG_ALBUMS:
                            fillMediaItems(mediaItems, Integer.toString(TYPE_ARTIST_ALL_SONGS) + Long.parseLong(parentId.substring(1)), "All songs", Uri.parse("android.resource://" +
                                    "bluros.music/drawable/ic_empty_music2"), "All songs by artist", MediaBrowser.MediaItem.FLAG_BROWSABLE);
                            List<Album> artistAlbums = ArtistAlbumLoader.getAlbumsForArtist(mContext, Long.parseLong(parentId.substring(1)));
                            for (Album album : artistAlbums) {
                                String songCount = MusicUtils.makeLabel(mContext, R.plurals.Nsongs, album.songCount);
                                fillMediaItems(mediaItems, Integer.toString(TYPE_ALBUM_SONGS) + Long.toString(album.id), album.title, MusicUtils.getAlbumArtUri(album.id), songCount, MediaBrowser.MediaItem.FLAG_BROWSABLE);
                        ...
                        case TYPE_ARTIST_ALL_SONGS:
                            ...
                            for (Song song : artistSongs) {
                                fillMediaItems(mediaItems, String.valueOf(song.id), song.title, MusicUtils.getAlbumArtUri(song.albumId), song.albumName, MediaBrowser.MediaItem.FLAG_PLAYABLE);
                            }
                            break;
                        case TYPE_PLAYLIST:
                            List<Playlist> playlistList = PlaylistLoader.getPlaylists(mContext, false);
                            for (Playlist playlist : playlistList) {
                                String songCount = MusicUtils.makeLabel(mContext, R.plurals.Nsongs, playlist.songCount);
                                fillMediaItems(mediaItems, Integer.toString(TYPE_PLAYLIST_ALL_SONGS) + Long.toString(playlist.id), playlist.name,
                                        Uri.parse("android.resource://" +
                                                "bluros.music/drawable/ic_empty_music2"), songCount, MediaBrowser.MediaItem.FLAG_BROWSABLE);
                            }
                            break;
